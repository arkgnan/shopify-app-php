...
$query = "SELECT * FROM recurring_billing WHERE shop_url='" . $shopify->get_url() . "' LIMIT 1";
...
if(isset($_GET['charge_id']) || $result->num_rows > 0) {
    ...
    $query = "INSERT INTO recurring_billing (shop_url, charge_id, gid, status) VALUES ('" . $shop_url . "','" . $charge_id . "','" . $gid . "','" . $status . "') ON DUPLICATE KEY UPDATE status='" . $status . "'";
    $mysql->query($query);
} else {
